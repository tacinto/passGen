...
btnGenerate.addEventListener('click', () => {
    passCopy.forEach(pass => {pass.textContent = 'Copy'})
    if (passLength.value <6 || passLength.value > 15){
        ...
        passAreas.forEach(passArea => passArea.textContent = randomPassword())
        enableCopy = true
    }
    copyPass()
})
...
function randomPassword() {
    ...
    for (let i = 0; i < passLength.value; i++) {
        ...
        switch (randNum) {
            case 0:
                randPass += `${upperCase[Math.floor(Math.random() * upperCase.length)]}`
                break
            case 1:
                randPass += `${lowerCase[Math.floor(Math.random() * lowerCase.length)]}`
                break
            case 2:
